...
namespace Surtr.Bytecode.Emit
{
    ...
    /// <summary>
    /// Names one of the host globals a module imported as a <c>native</c> variable.
    /// </summary>
    /// <remarks>
    /// Module-local, not runtime-wide: the index is into the module's own import table, which is
    /// bound to the host's globals by name when the module is loaded. That is what lets the same
    /// compiled module load into two runtimes that number their globals differently.
    /// </remarks>
    public readonly struct SurtrNativeVariableToken : IEquatable<SurtrNativeVariableToken>
    {
        private readonly int _value;

        internal SurtrNativeVariableToken(int index) => _value = index + 1;

        /// <summary>The module-local import index this token names.</summary>
        public int Index
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => _value - 1;
        ...
        /// <summary>Whether this token names an import at all.</summary>
        public bool IsValid
        {
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            get => _value != 0;
        ...
        /// <inheritdoc/>
        public bool Equals(SurtrNativeVariableToken other) => _value == other._value;

        /// <inheritdoc/>
        public override bool Equals(object? obj) => obj is SurtrNativeVariableToken other && Equals(other);

        /// <inheritdoc/>
        public override int GetHashCode() => _value;

        /// <inheritdoc/>
        public override string ToString() => $"nativeVar#{Index}";
    }

    /// <summary>Names one of the host functions a module imported as a <c>native</c> function.</summary>
    /// <remarks>Module-local for the same reason <see cref="SurtrNativeVariableToken"/> is.</remarks>
    public readonly struct SurtrNativeFunctionToken : IEquatable<SurtrNativeFunctionToken>
    {
        private readonly int _value;

        internal SurtrNativeFunctionToken(int index) => _value = index + 1;
        ...
        /// <inheritdoc/>
        public bool Equals(SurtrNativeFunctionToken other) => _value == other._value;

        /// <inheritdoc/>
        public override bool Equals(object? obj) => obj is SurtrNativeFunctionToken other && Equals(other);

        /// <inheritdoc/>
        public override int GetHashCode() => _value;

        /// <inheritdoc/>
        public override string ToString() => $"nativeFun#{Index}";
    }

    /// <summary>Names an entry in a chunk's type access table.</summary>
