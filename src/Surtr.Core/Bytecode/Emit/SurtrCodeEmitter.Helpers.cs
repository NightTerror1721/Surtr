...
namespace Surtr.Bytecode.Emit
{
    ...
    public sealed partial class SurtrCodeEmitter
    {
        ...
        /// <summary>Reads a host-defined global variable, in the narrowest encoding that reaches it.</summary>
        public SurtrCodeEmitter LoadGlobal(int index)
            => index <= ushort.MaxValue ? Ldg(index) : LdgX(index);
        ...
        public SurtrCodeEmitter LoadGlobal(SurtrNativeGlobalVariable variable)
            => LoadGlobal(RequireRegistered(variable));

        /// <summary>Writes a host-defined global variable, in the narrowest encoding that reaches it.</summary>
        public SurtrCodeEmitter StoreGlobal(int index)
            => index <= ushort.MaxValue ? Stg(index) : StgX(index);

        /// <summary>Writes a host-defined global variable.</summary>
        ...
        public SurtrCodeEmitter StoreGlobal(SurtrNativeGlobalVariable variable)
        {
            ...
            return StoreGlobal(RequireRegistered(variable));
        }

        private static int RequireRegistered(SurtrNativeGlobalVariable variable)
        {
            if (variable is null)
                throw new ArgumentNullException(nameof(variable));

            if (variable.Index < 0)
                throw new ArgumentException($"Host global '{variable.Name}' has not been registered with a runtime, so it has no index to compile against.", nameof(variable));
        ...
        public SurtrCodeEmitter CallGlobal(SurtrNativeGlobalFunction function, bool discardResult = false)
        {
            ...
            if (function.Index < 0)
                throw new ArgumentException($"Host global function '{function.Name}' has not been registered with a runtime, so it has no index to compile against.", nameof(function));

            int results = discardResult || function.ReturnType.Reference.TypeCode.IsVoid ? 0 : 1;

            return function.Index <= ushort.MaxValue
                ? CallGlobalNative(function.Index, function.ParameterCount, results)
                : CallGlobalNativeX(function.Index, function.ParameterCount, results);
        }
