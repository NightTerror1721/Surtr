...
| [`Genericos-Borrado-Evaluacion-Y-Alternativas.md`](Genericos-Borrado-Evaluacion-Y-Alternativas.md) | Evaluación completa del borrado de genéricos: cómo funciona (niveles a/b/c de Compiler-Plan §8), dónde se borra exactamente en el emisor, qué cuesta medido en el VM (box-in/cast-out; `generics` 29.5× C#) y qué no cuesta (despacho, colecciones). Alternativas para no borrar (monomorfización, reificación, stencil, especialización dirigida) con pros/contras, cambios por capa, coste estimado por fases y recomendación. |
