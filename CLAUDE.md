...
The instruction set, the metadata layer, the object registry, the runtime object model, the built-in classes, the standard library, the interpreter and the emitter exist. **The compiler goes the whole way**: it lexes, parses, binds, lowers, emits a module onto `SurtrModuleBuilder`, and writes it as a `.surtrc` image — and `src/Surtr.Tests/Compiler/CodeGen/ModuleEmitterTests.cs` takes Surtr source through all of it, loads the result into a real `SurtrRuntime`, and runs it. It is not finished against the specification, and **`docs/Compiler-Plan.md` §10.2 is the authoritative list of what it still owes** — currently nothing: the last item there, the standard library written in Surtr, is closed by `src/Surtr.Stdlib`, which the solution compiles from `.surtr` sources on every build. §10.1 is the other half of that audit: eight constructs that compiled and answered *wrongly*, all closed, and worth reading once because most were one mistake — **a node the parser produced that nothing downstream read**. Nothing in a build complains about that, so a new construct is not done when it parses; it is done when something asks for it. §10.1b is generics going from declarable to usable, which was the largest single item on the list, §10.1c is a lambda taking its parameter types from the parameter it is passed to, §10.1d is accessibility going from decoration to rule, §10.1e the five smaller closures — interface nesting and properties, exhaustive switches, `operator[]`, attributes — §10.1f the build model, and §10.1g flow analysis, initializer ordering, emit diagnostics with a span, and the two places the specification disagreed with itself.

**Everything `docs/VM-Plan.md` §4 asked the runtime for is implemented**: parameter defaults and varargs, `sealed`, enums with per-case ordinals, generic parameters on the built-ins, nullable primitives, `range`, the standard library with its exception hierarchy and the trap-to-class mapping, per-module native imports bound by name at load, class-naming boxing for value classes, attributes as real classes, and an instruction budget. **§4.8 — the compiler side, the list of things the runtime assumes and will not check — is met item by item too**, and `docs/Compiler-Plan.md` §7 reproduces it from the compiler's end. The list is worth reading as what the runtime is *entitled to assume*: anything on it that stops holding is a miscompile, not a missing feature. For what the front end still owes, `docs/Compiler-Plan.md` §10.2 is the maintained list — not §4.8.
...
This project is indexed by GitNexus as **Surtr** (9446 symbols, 38300 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
