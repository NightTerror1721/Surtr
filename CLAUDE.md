...
**A module-level `native` declaration is a host global, not a member** (§10). It gets no static slot and no method-table entry: the emitter interns it into the module's native import table and use sites emit `Ldg`/`Stg`/`CallGlobalNative`, so a name the host never registered fails the load rather than reading a zero. A `native` member *inside a class* is the other thing entirely — a body published by link name (`DeclareNativeMethod`), which is how a host writes the standard library.
...
**A `singleton` is a class plus one static, and that static is the feature.** §1.1 puts type names and value names in separate namespaces, so `Registry` resolves as a *type* — reading it as a value means going through the synthetic `$instance$Registry` its own initializer builds, which is the one place in the binder a type name answers as one. §2.8 rejects a constructor, since nothing would choose when to run it. A **`value class`** goes the other way: a call on one boxes with `BoxAs` and `this` inside the callee unwraps, so the single rule is *inside a value class's own method the receiver is boxed, everywhere else a value class is its field* — which is what keeps `id.raw` free at every other site. §6.3's "a direct call does not box" is a missed optimisation rather than a correctness gap; boxing more than needed is safe where boxing less is a type confusion.
